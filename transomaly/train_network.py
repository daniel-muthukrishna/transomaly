...
def train_model(X_train, X_test, y_train, y_test, yerr_train, yerr_test, fig_dir='.', epochs=20, retrain=False,
                passbands=('g', 'r'), model_change='', reframe=False, probabilistic=False):
    ...
    if not retrain and os.path.isfile(model_filename):
        if probabilistic:
            ...
            with sess.as_default():
                model = load_model(model_filename, custom_objects={'loss': negloglik})
        else:
...
def main():
    ...
    model, model_name, tf_sess = train_model(X_train, X_test, y_train, y_test, yerr_train, yerr_test, fig_dir=fig_dir, epochs=train_epochs,
                        retrain=retrain, passbands=passbands, model_change=nn_architecture_change, reframe=reframe_problem, probabilistic=probabilistic)
